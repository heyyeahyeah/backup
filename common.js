...
/*▼カード一括選択on,off▼*/
	...
	$(window).load(function() {
		...
		var on_css_small = {"transform":"scale(0.9,0.9)","-webkit-transform":"scale(0.9,0.9)"};
...
/* タイトル文字サイズ */
	...
	if(text_length_14.text().length>22) {
		text_length_14.css({'font-size':'10px', 'letter-spacing': '-0.4px'});
	}
...
/*ﾎﾞﾀﾝ凹み*/
	$('.bt_switch').on(start, function(){ $(this).addClass('dent'); }).on(end, function(){ $(this).removeClass('dent'); }).on('mouseout', function(){ $(this).removeClass('dent'); });
//	$('.bt_switch_parent a').on(start, function(){ var className = $(this).attr('class');$(this).removeClass().addClass(className+'_gray'); }).on(end, function(){ var className = $(this).attr('class');$(this).removeClass().addClass( className.replace('_gray','') ); }).on('mouseout', function(){ var className = $(this).attr('class');$(this).removeClass().addClass( className.replace('_gray','') ); });
	$('body').on(end, function(){ $('.dent').removeClass('dent'); });
});
...
var number_format = function(num){
	var num = num.toString(), number = "", count = 0;
	for(var n = num.length-1 ; n >= 0 ; n-- ){ number = (++count%3 == 0 && num.length != count ? ",":"") + num[n] + number; }
	return number;
}
;(function($){
	$.ibox = function(el,options){
		...
		el.click(function(){
			...
			vars.$clone.css({ 'margin-left' : 'auto' , 'margin-right': 'auto' , 'margin-top' : (dcTop > 10 ? dcTop: 10) +'px' });
	...
	function toggleMenu() {
		...
		if (menu.length) {
			$('html,body').animate({scrollTop:$('#headerNaviPetitOn').position().top}, '250');
		...
		if (workMenu.length) {
			$('html,body').animate({scrollTop:$('#wrap').position().top}, '250');
