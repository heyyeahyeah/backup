	...
	$.iSlider = function(el, options) {
		...
		methods= {
			init : function(){
				...
				length = container.css({ 'float':'left' ,'display':'block' }).length;
				
				if(length<=1) vars.slideshowSpeed = 0;
				...
				container = el.find(vars.elem+' > li');
				var c_css =  { "overflow": "hidden", "position": vars.position ,"width" : vars.width };
				...
				if(!height) height = parseInt( container.css('height') );
				...
				list.css( engine+'transform' , "translate3d(-"+(startSlide*width)+"px, 0,0)")
				.css( engine+"transition-duration" ,"0ms" );
			...
			move : function(e) {
				...
				if(zoom_end[$currentSlide.val()] > 1){
					if ( methods.zoom_point_move(e) ) return;
				}
				if (!touch) return;
			...
			set_zoom_assign : function(){
				...
				if(zoom_margin[0]<0) zoom_margin[0]=0;
				if(zoom_margin[1]<0) zoom_margin[1]=0;

				if (!zoom_end[$currentSlide.val()]) zoom_end[$currentSlide.val()] = 1;

				var $zoom_img = container.eq($currentSlide.val()).find('img');
				if(!left[$currentSlide.val()]) left[$currentSlide.val()] = parseInt($zoom_img.css('margin-left'))+((width * zoom_end[$currentSlide.val()]) - width)*0.5;
				if(!top[$currentSlide.val()])top[$currentSlide.val()] = parseInt($zoom_img.css('margin-top'))+((height * zoom_end[$currentSlide.val()]) - height)*0.5;
			},
			...
			zoom_move : function(e){
				...
				$zoom_img.css({ 'height': (height * zoomin)+'px' , 'width': (width * zoomin)+'px' });
				...
				if (zoomin == zoom){ 
					var x =  parseInt($zoom_img.css('margin-left')) , y = parseInt($zoom_img.css('margin-top'));
				}else if(zoom_end[$currentSlide.val()] > zoomin){
					var x1 = left[$currentSlide.val()] ,x2 = 0 , y1 = -top[$currentSlide.val()] ,y2 =0;
					...
					var x1 = zoom_margin[0]*zoomin  ,x2 = width*0.5*zoomin, y1 = -1 * zoom_margin[1]*zoomin, y2 = -1* height*0.5*zoomin;
					var vector = trimetric.unit_vector(x1,x2, y1 ,y2);
					var x = ((current_zoom - zoom_start_length)*vector[0])+  center_horizontal,
					y = - ((current_zoom - zoom_start_length)*vector[1])+ center_vertical;
				...
				$zoom_img.css({ "margin-left": x +"px", "margin-top": y+"px" });
			...
			zoom_point_move :function(e){
				...
				var moveX = currentX - pointX, moveY = currentY - pointY; 
		...
		if (!isTouch){
			...
			el.bind('gestureend', function(e) {
				if (zoomin) zoom_end[$currentSlide.val()] = zoomin;
				//zoomin = 1;
			});
			...
			el.bind('touchmove', function(e) {
				...
				if (event.changedTouches[0].identifier != identifier ) return;
				methods.move(e);
			});
			el.bind('touchend', function(e) {
				...
				if(zoom_margin.length) { zoom_margin = []; }
				if (event.changedTouches[0].identifier != identifier ) return;
		...
		$('img').on('dragstart', function(event) {  if(!isTouch){ event.preventDefault(); } });
